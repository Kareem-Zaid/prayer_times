...
class LocalNotifsService {
  ...
  Future<void> zonedScheduleNotification(
    ...
    await _flutterLocalNotificationsPlugin.zonedSchedule(
      ...
      matchDateTimeComponents:
          DateTimeComponents.dayOfWeekAndTime, // Repeat yearly
    );
  }
  ...
  Future<List<PendingNotificationRequest>> _getPendingNotifications() async {
    ...
    debugPrint('Pending notifications count: ${pendingNotifs.length}');
  ...
  void schedulePrayerNotifications(UserSettings settings) async {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      for (int i = 0; i < prayerList.length; i++) {
        ...
        final pendingNotifs = await _getPendingNotifications();

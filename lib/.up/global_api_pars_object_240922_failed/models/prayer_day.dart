class PrayerYear {
  final int code;
  final String status;
  final Map<String, List<Data>> dataYear;

  PrayerYear(
      {required this.code, required this.status, required this.dataYear});

  factory PrayerYear.fromJson(Map<String, dynamic> json) {
    return PrayerYear(
      code: json['code'],
      status: json['status'],
      dataYear: Map.from(json['data']).map((k, v) {
        return MapEntry(
          k,
          List.from((v as List).map((datum) {
            return Data.fromJson(datum);
          })),
        );
      }),
    );
...
class PrayerMonth {
  final int code;
  final String status;
  final List<Data> dataMonth;

  PrayerMonth(
      {required this.code, required this.status, required this.dataMonth});
  factory PrayerMonth.fromJson(Map<String, dynamic> json) {
    return PrayerMonth(
      code: json['code'],
      status: json['status'],
      dataMonth: List.from((json['data'] as List).map((datum) {
        return Data.fromJson(datum);
      })),
    );
  }

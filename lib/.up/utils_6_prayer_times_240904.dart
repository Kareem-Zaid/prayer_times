...
class Utils {
  ...
  static Prayer getNextPrayer({
    ...
    final DateTime ishaTime = parseTime(prayers.prayerList.last.time);
    final DateTime fajrTime =
        parseTime(prayers.prayerList.first.time).add(const Duration(days: 1));

    if (!prayerAssigned && (now.isAfter(ishaTime) || now.isBefore(fajrTime))) {
      nextPrayer = prayers.prayerList.first;
    }

...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _schedulePrayerNotifications() async {
    ...
    for (var monthStr in yearData.keys) {
      List<Future<void>> futures = []; // A new list of futures for each month
      ...
      for (var dayData in yearData[monthStr]!) {
        ...
        for (var prayer in prayerList) {
          futures.add(_localNotifs.zonedScheduleNotification(
            prayer.time,
            '',
            '',
          ));
      ...
      // Await scheduling the current month before proceeding to the next
      await Future.wait(futures);
      debugPrint('Scheduled notifications for month $monthStr');
      debugPrint('# of scheduled notifications: ${_localNotifs.id}');
    }

    debugPrint('All notifications scheduled for the year.');
  }
